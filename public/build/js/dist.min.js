...
}]),angular.module("template/typeahead/typeahead-match.html",[]).run(["$templateCache",function(a){a.put("template/typeahead/typeahead-match.html",'<a tabindex="-1" bind-html-unsafe="match.label | typeaheadHighlight:query"></a>')}]),angular.module("template/typeahead/typeahead-popup.html",[]).run(["$templateCache",function(a){a.put("template/typeahead/typeahead-popup.html","<ul class=\"dropdown-menu\" ng-style=\"{display: isOpen()&&'block' || 'none', top: position.top+'px', left: position.left+'px'}\">\n"+'    <li ng-repeat="match in matches" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)" ng-click="selectMatch($index)">\n        <div typeahead-match index="$index" match="match" query="query" template-url="templateUrl"></div>\n    </li>\n</ul>')}]),angular.module("mtgshed",["ngCookies","ngResource","ui.bootstrap","ui.router","mtgshed.system","mtgshed.articles"]),angular.module("mtgshed.system",[]),angular.module("mtgshed.articles",[]),angular.module("mtgshed").config(["$stateProvider","$urlRouterProvider",function(a,b){b.otherwise("/"),a.state("all articles",{url:"/articles",templateUrl:"views/articles/list.html"}).state("create article",{url:"/articles/create",templateUrl:"views/articles/create.html"}).state("edit article",{url:"/articles/:articleId/edit",templateUrl:"views/articles/edit.html"}).state("article by id",{url:"/articles/:articleId",templateUrl:"views/articles/view.html"}).state("home",{url:"/",templateUrl:"views/index.html"})}]),angular.module("mtgshed").config(["$locationProvider",function(a){a.hashPrefix("!")}]),angular.module("mean.articles").controller("ArticlesController",["$scope","$stateParams","$location","Global","Articles",function(a,b,c,d,e){a.global=d,a.create=function(){var a=new e({title:this.title,content:this.content});a.$save(function(a){c.path("articles/"+a._id)}),this.title="",this.content=""},a.remove=function(b){if(b){b.$remove();for(var d in a.articles)a.articles[d]===b&&a.articles.splice(d,1)}else a.article.$remove(),c.path("articles")},a.update=function(){var b=a.article;b.updated||(b.updated=[]),b.updated.push((new Date).getTime()),b.$update(function(){c.path("articles/"+b._id)})},a.find=function(){e.query(function(b){a.articles=b})},a.findOne=function(){e.get({articleId:b.articleId},function(b){a.article=b})}}]),angular.module("mean.system").controller("HeaderController",["$scope","Global",function(a,b){a.global=b,a.menu=[{title:"Articles",link:"articles"},{title:"Create New Article",link:"articles/create"}],a.isCollapsed=!1}]),angular.module("mtgshed.system").controller("IndexController",["$scope","Global",function(a,b){a.global=b}]),angular.element(document).ready(function(){"#_=_"===window.location.hash&&(window.location.hash="#!"),angular.bootstrap(document,["mtgshed"])}),angular.module("mtgshed.articles").factory("Articles",["$resource",function(a){return a("articles/:articleId",{articleId:"@_id"},{update:{method:"PUT"}})}]),angular.module("mtgshed.system").factory("Global",[function(){var a=this;return a._data={user:window.user,authenticated:!!window.user},a._data}]);